...
"
" | Easily | manageable | text  | tables | Implementation |
" |--------|------------|-------|--------|----------------|
"
" Original code credit: https://github.com/vimwiki/vimwiki/
...
function! pandoc#table#Init()
    ...
    command! -buffer PandocTableAlignW call pandoc#table#align_or_cmd('gww')
    command! -buffer PandocTableMoveColumnLeft call pandoc#table#move_column_left()
    ...
    nnoremap <buffer> gqq :PandocTableAlignQ<CR>
    nnoremap <buffer> gww :PandocTableAlignW<CR>
    if !hasmapto('<Plug>PandocTableMoveColumnLeft')
        nmap <silent><buffer> <A-Left> <Plug>PandocTableMoveColumnLeft
    endif
    nnoremap <silent><script><buffer> <Plug>PandocTableMoveColumnLeft :PandocTableMoveColumnLeft<CR>
    if !hasmapto('<Plug>PandocTableMoveColumnRight')
        nmap <silent><buffer> <A-Right> <Plug>PandocTableMoveColumnRight
    endif
    nnoremap <silent><script><buffer>
                \ <Plug>PandocTableMoveColumnRight :PandocTableMoveColumnRight<CR>

    " Table autocmds
    au InsertLeave *.pandoc,*.pdk,*.pd,*.pdc call pandoc#table#format(line("."))
    au InsertEnter *.pandoc,*.pdk,*.pd,*.pdc call pandoc#table#reset_tw(line("."))

    if g:pandoc#filetypes#pandoc_markdown == 1
        au InsertLeave *.markdown,*.mdown,*.mkd,*.mkdn,*.mdwn,*.md call pandoc#table#format(line("."))
        au InsertEnter *.markdown,*.mdown,*.mkd,*.mkdn,*.mdwn,*.md call pandoc#table#reset_tw(line("."))
    endif
endfunction
...
function! pandoc#table#get_cells(line)
    let result = []
    let cell = ''
    let quote = ''
    let state = 'NONE'
    ...
    for idx in range(strlen(a:line))
        ...
        elseif state ==# 'CELL'
            if ch == '[' || ch == '{'
                ...
                let quote = ch
            elseif ch == '|'
                call add(result, pandoc#table#trim(cell))
                let cell = ""
            else
                let cell .= ch
            endif
    ...
    if cell.quote != ''
        call add(result, pandoc#table#trim(cell.quote, '|'))
    endif
    return result
endfunction
...
function! s:col_count(lnum)
    return len(pandoc#table#get_cells(getline(a:lnum)))
endfunction
...
function! s:get_cell_max_lens(lnum, ...)
    ...
    for [lnum, row] in s:get_rows(a:lnum)
        ...
        let cells = a:0 > 1 ? a:1[lnum - a:2] : pandoc#table#get_cells(row)
...
function! s:get_aligned_rows(lnum, col1, col2)
    ...
    for [lnum, row] in rows
        call add(cells, pandoc#table#get_cells(row))
    endfor
...
function! s:kbd_create_new_row(cols, goto_first)
    ...
    let cmd .= "\<ESC>:call pandoc#table#format(line('.'))\<CR>"
    let cmd .= "\<ESC>0"
    if a:goto_first
...
function! pandoc#table#goto_next_col()
    let curcol = virtcol('.')
    let lnum = line('.')
    ...
    call pandoc#table#cursor(lnum, newcol)
endfunction
...
function! s:kbd_goto_next_col(jumpdown)
    ...
    let cmd .= ":call pandoc#table#goto_next_col()\<CR>a"
    return cmd
endfunction


" Used in s:kbd_goto_prev_col
function! pandoc#table#goto_prev_col()
    let curcol = virtcol('.')
    let lnum = line('.')
    let newcol = s:get_indent(lnum)
    let max_lens = s:get_cell_max_lens(lnum)
...
function! s:kbd_goto_prev_col(jumpup)
    ...
    let cmd .= ":call pandoc#table#goto_prev_col()\<CR>a"
...
function! pandoc#table#kbd_cr()
    ...
    if s:is_separator(getline(lnum+1)) || !s:is_table(getline(lnum+1))
        let cols = len(pandoc#table#get_cells(getline(lnum)))
...
function! pandoc#table#kbd_shift_tab()
    let lnum = line('.')
    if !s:is_table(getline(lnum))
        return "\<S-Tab>"
    endif
...
function! pandoc#table#format(lnum, ...)
    if !(&filetype ==? 'pandoc')
        return
    endif
...
function! pandoc#table#align_or_cmd(cmd)
    if s:is_table(getline('.'))
        call pandoc#table#format(line('.'))
    else
        exe 'normal! '.a:cmd
    endif
endfunction
...
function! pandoc#table#reset_tw(lnum)
    if !(&filetype ==? 'pandoc')
        return
    endif
    let line = getline(a:lnum)
...
function! pandoc#table#move_column_left()
    ...
    if cur_col > 0
        call pandoc#table#format(line('.'), cur_col-1, cur_col)
        call cursor(line('.'), 1)
...
function! pandoc#table#move_column_right()
    ...
    if cur_col < s:col_count(line('.'))-1
        call pandoc#table#format(line('.'), cur_col, cur_col+1)
        call cursor(line('.'), 1)
...
function! pandoc#table#get_rows(lnum)
    return s:get_rows(a:lnum)
endfunction


function! pandoc#table#is_table(line)
    return s:is_table(a:line)
endfunction


function! pandoc#table#is_separator(line)
    return s:is_separator(a:line)
endfunction


function! pandoc#table#cell_splitter()
    return s:cell_splitter()
endfunction


function! pandoc#table#sep_splitter()
    return s:sep_splitter()
endfunction

function! pandoc#table#trim(string, ...)
    let chars = ''
    if a:0 > 0
        let chars = a:1
    endif
...
function! pandoc#table#cursor(lnum, cnum)
    exe a:lnum
    exe 'normal! 0'.a:cnum.'|'
endfunction
